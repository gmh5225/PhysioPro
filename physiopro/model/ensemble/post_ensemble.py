    ...
    def forward(self, data):
        ...
        if self.ensemble_type == "average_ensemble":
            ensemble_pred = torch.cat(
                [torch.sum(preds[:, i::class_type], dim=1).unsqueeze(1) for i in range(class_type)], dim=1
            )
            ensemble_pred = torch.softmax(ensemble_pred, dim=-1)
        else:
            ensemble_weights = self.network(features)
            ensemble_pred = [
                torch.sum(preds[:, i::class_type] * ensemble_weights, dim=1).unsqueeze(1) for i in range(class_type)
            ]
